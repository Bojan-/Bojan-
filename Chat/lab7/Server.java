...
public class Server implements HttpHandler {
    ...
    private void doPost(HttpExchange httpExchange) {
        try {
	    MessageExchange.Line line = messageExchange.getClientMessage(httpExchange.getRequestBody());
	    System.out.println(line.id + " " + line.user + ": " + line.message);
            history.add("id: " + line.id + ", " + "user: " + line.user + ", " + "message" + ": " + line.message);
	...
	try {
	    ...
	    for (String i : history) {
	    	if (i.regionMatches(4, id, 0, id.length()))
		    history.remove(i);
	    }
	} catch (ParseException e) {
            System.err.println("Invalid user message: " + httpExchange.getRequestBody() + " " + e.getMessage());
    ...
    private void doChange(HttpExchange httpExchange) {
	try {
	    MessageExchange.Line line = messageExchange.getClientMessage(httpExchange.getRequestBody());
	    for (String i : history) {
	    	if (i.regionMatches(4, line.id, 0, line.id.length())) {
		    int index = history.indexOf(i);
		    String sub = "";
		    for (int j = 9; j < i.length(); j++)
			if (i.charAt(j - 2) == ':' && i.charAt(j - 9) == 'm')
			    sub = i.substring(j, i.length());
		    i = i.replaceAll(sub, line.message);
		    history.set(index, i);
		}
	    }
	} catch (ParseException e) {
